# encoding: utf-8
RSpec.describe DoubleEntry::Reporting::Aggregate do
  let(:user) { User.make! }
  ...
  it 'should store the aggregate for quick retrieval' do
    DoubleEntry::Reporting::Aggregate.new(:sum, :savings, :bonus, :range => DoubleEntry::Reporting::TimeRange.make(:year => 2009, :month => 10)).amount
    expect(DoubleEntry::Reporting::LineAggregate.count).to eq 1
  end

  it 'should only store the aggregate once if it is requested more than once' do
    DoubleEntry::Reporting::Aggregate.new(:sum, :savings, :bonus, :range => DoubleEntry::Reporting::TimeRange.make(:year => 2009, :month => 9)).amount
    DoubleEntry::Reporting::Aggregate.new(:sum, :savings, :bonus, :range => DoubleEntry::Reporting::TimeRange.make(:year => 2009, :month => 9)).amount
    DoubleEntry::Reporting::Aggregate.new(:sum, :savings, :bonus, :range => DoubleEntry::Reporting::TimeRange.make(:year => 2009, :month => 10)).amount
    expect(DoubleEntry::Reporting::LineAggregate.count).to eq 2
  end
  ...
  it 'should calculate the complete year correctly' do
    expect(
      DoubleEntry::Reporting::Aggregate.new(
        :sum, :savings, :bonus,
        :range => DoubleEntry::Reporting::TimeRange.make(:year => 2009)
      ).formatted_amount,
  ...
  it 'should calculate seperate months correctly' do
    expect(
      DoubleEntry::Reporting::Aggregate.new(
        :sum, :savings, :bonus,
        :range => DoubleEntry::Reporting::TimeRange.make(:year => 2009, :month => 10)
      ).formatted_amount,
    ).to eq Money.new(110_00)
    expect(
      DoubleEntry::Reporting::Aggregate.new(
        :sum, :savings, :bonus,
        :range => DoubleEntry::Reporting::TimeRange.make(:year => 2009, :month => 11)
      ).formatted_amount,
  ...
  it 'should calculate seperate weeks correctly' do
    ...
    expect(
      DoubleEntry::Reporting::Aggregate.new(
        :sum, :savings, :bonus,
        :range => DoubleEntry::Reporting::TimeRange.make(:year => 2009, :week => 40)
      ).formatted_amount,
  ...
  it 'should calculate seperate days correctly' do
    ...
    expect(
      DoubleEntry::Reporting::Aggregate.new(
        :sum, :savings, :bonus,
        :range => DoubleEntry::Reporting::TimeRange.make(:year => 2009, :week => 44, :day => 7)
      ).formatted_amount,
    ).to eq Money.new(90_00)
  end
  ...
  it 'should calculate seperate hours correctly' do
    ...
    expect(
      DoubleEntry::Reporting::Aggregate.new(
        :sum, :savings, :bonus,
        :range => DoubleEntry::Reporting::TimeRange.make(:year => 2009, :week => 44, :day => 7, :hour => 0)
      ).formatted_amount,
    ).to eq Money.new(40_00)
    expect(
      DoubleEntry::Reporting::Aggregate.new(
        :sum, :savings, :bonus,
        :range => DoubleEntry::Reporting::TimeRange.make(:year => 2009, :week => 44, :day => 7, :hour => 1)
      ).formatted_amount,
  ...
  it 'should calculate, but not store aggregates when the time range is still current' do
    Timecop.freeze Time.local(2009, 11, 21) do
      ...
      expect(DoubleEntry::Reporting::LineAggregate.count).to eq 0
    end
  end
  ...
  it 'should calculate, but not store aggregates when the time range is in the future' do
    Timecop.freeze Time.local(2009, 11, 21) do
      expect(
        DoubleEntry::Reporting::Aggregate.new(
          :sum, :savings, :bonus,
          :range => DoubleEntry::Reporting::TimeRange.make(:year => 2009, :month => 12)
        ).formatted_amount,
      ).to eq Money.new(0)
      expect(DoubleEntry::Reporting::LineAggregate.count).to eq 0
    end
  ...
  it 'should calculate monthly all_time ranges correctly' do
    expect(
      DoubleEntry::Reporting::Aggregate.new(
        :sum, :savings, :bonus,
        :range => DoubleEntry::Reporting::TimeRange.make(:year => 2009, :month => 12, :range_type => :all_time)
      ).formatted_amount,
    ).to eq Money.new(200_00)
  end
  ...
  it 'should calculate weekly all_time ranges correctly' do
    expect(
      DoubleEntry::Reporting::Aggregate.new(
        :sum, :savings, :bonus,
        :range => DoubleEntry::Reporting::TimeRange.make(:year => 2009, :week => 43, :range_type => :all_time)
      ).formatted_amount,
    ).to eq Money.new(110_00)
  end
  ...
  it 'raises an DoubleEntry::Reporting::AggregateFunctionNotSupported exception' do
    expect do
      DoubleEntry::Reporting::Aggregate.new(
        :not_supported_calculation, :savings, :bonus,
        :range => DoubleEntry::Reporting::TimeRange.make(:year => 2009, :week => 43, :range_type => :all_time)
      ).amount
    end.to raise_error(DoubleEntry::Reporting::AggregateFunctionNotSupported)
  end

  context 'filters' do
    let(:range) { DoubleEntry::Reporting::TimeRange.make(:year => 2011, :month => 10) }
    ...
    it 'saves filtered aggregations' do
      expect do
        DoubleEntry::Reporting::Aggregate.new(:sum, :savings, :bonus, :range => range, :filter => filter).amount
      end.to change { DoubleEntry::Reporting::LineAggregate.count }.by 1
    end
    ...
    it 'saves filtered aggregations and non filtered aggregations separately' do
      expect do
        DoubleEntry::Reporting::Aggregate.new(:sum, :savings, :bonus, :range => range, :filter => filter).amount
        DoubleEntry::Reporting::Aggregate.new(:sum, :savings, :bonus, :range => range).amount
      end.to change { DoubleEntry::Reporting::LineAggregate.count }.by 2
    end
    ...
    it 'loads the correct saved aggregation' do
      ...
      expect(
        DoubleEntry::Reporting::Aggregate.new(:sum, :savings, :bonus, :range  => range, :filter => filter).formatted_amount,
      ).to eq Money.new(10_00)
      expect(
        DoubleEntry::Reporting::Aggregate.new(:sum, :savings, :bonus, :range  => range).formatted_amount,
      ).to eq Money.new(19_00)
    end
  end
end

RSpec.describe DoubleEntry::Reporting::Aggregate, 'currencies' do
  let(:user) { User.make! }
  before do
  ...
  it 'should calculate the sum in the correct currency' do
    expect(
      DoubleEntry::Reporting::Aggregate.new(
        :sum, :btc_savings, :btc_test_transfer, :range => DoubleEntry::Reporting::TimeRange.make(:year => Time.now.year)
      ).formatted_amount,
